...
/**
 * REST controller for managing {@link com.cenfotec.jtestproject.domain.Invoice}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class InvoiceResource {
    ...
    public List<Invoice> getAllInvoices(@RequestParam(required = false) String filter) {
        if ("license-is-null".equals(filter)) {
            log.debug("REST request to get all Invoices where license is null");
            return StreamSupport
                .stream(invoiceRepository.findAll().spliterator(), false)
                .filter(invoice -> invoice.getLicense() == null)
