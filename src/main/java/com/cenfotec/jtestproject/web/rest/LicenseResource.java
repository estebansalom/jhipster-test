...
/**
 * REST controller for managing {@link com.cenfotec.jtestproject.domain.License}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class LicenseResource {
    ...
    public ResponseEntity<License> updateLicense(@PathVariable(value = "id", required = false) final Long id, @RequestBody License license)
     ...
     */
    @GetMapping("/licenses")
    public List<License> getAllLicenses(@RequestParam(required = false) String filter) {
        if ("userdetails-is-null".equals(filter)) {
            log.debug("REST request to get all Licenses where userDetails is null");
            return StreamSupport
                .stream(licenseRepository.findAll().spliterator(), false)
                .filter(license -> license.getUserDetails() == null)
